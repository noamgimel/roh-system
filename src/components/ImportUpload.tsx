...
export default function ImportUpload() {
  ...
  async function handleCommit() {
    ...
    try {
      ...
      setPreview(null);
      setFile(null);
      router.refresh(); // השורות הממתינות להשלמה מופיעות בהמשך העמוד
    } catch (e) {
    ...
    <div>
      ...
      {report && (
        <div className="bg-white rounded-xl border border-green-200 p-6 max-w-2xl">
          ...
          {report.pendingSaved > 0 && (
            <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              <b>{report.pendingSaved}</b> שורות לא יובאו (בדרך כלל חסר ת&quot;ז) — הן
              ממתינות להשלמה <a href="#pending" className="underline font-medium">בהמשך העמוד</a>,
              ויישארו שם עד שתשלים או תמחק אותן.
            </div>
          )}
          {report.skippedByName > 0 && (
            <div className="mt-2 text-xs text-slate-500">
              {report.skippedByName} שורות שגויות דולגו כי לקוח באותו שם כבר קיים (הושלם בעבר).
            </div>
          )}
          <Link
            href="/clients"
